...
from project_utils.general_utils import (
    init_experiment,
    get_mean_lr,
    str2bool,
...
def train(
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            # Total loss
            loss = (
                1 - args.sup_con_weight
            ) * contrastive_loss + args.sup_con_weight * sup_con_loss
        ...
        with torch.no_grad():
            ...
            all_acc_test, old_acc_test, new_acc_test = test_kmeans(
                model, test_loader, epoch=epoch, save_name="Test ACC", args=args
            )
        ...
        if old_acc_test > best_test_acc_lab:
            ...
            print(
                f"Best ACC on old Classes on disjoint test set: {old_acc_test:.4f}..."
            )
            print(
                "Best Train Accuracies: All {:.4f} | Old {:.4f} | New {:.4f}".format(
                    all_acc, old_acc, new_acc
                )
            )
            ...
            torch.save(
                ...
                args.model_path[:-3] + f"_proj_head_best.pt",
            )
            print(
                "projection head saved to {}.".format(
                    args.model_path[:-3] + f"_proj_head_best.pt"
